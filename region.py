...
class Region:
    ...
    # default values
    # JMT: this is the size of an mcregion file
    tilesize = 1024
    scale = 6
    vscale = 6
    trim = 0
    ...
    def unzipfiles(self, layertype, zipfile):
        ...
        templates = {'elevation': ['%s', 'img%s_13'],
                     'landcover': ['%s']}
        suffixes = {'elevation': [['img', 'img.aux.xml'], ],
                    'landcover': [['tif', 'tfw'], ['img', 'ige'], ]}
    ...
    def build_map(self, wantCL=True, do_pickle=False):
        ...
        elvrt = os.path.join(self.mapsdir, 'elevation.vrt')
        elfile = os.path.join(self.mapsdir, 'elevation.tif')
        elextents = self.albersextents['elevation']
        warpcmd = 'gdalwarp -q -multi -t_srs "%s" -tr %d %d -te %d %d %d %d -r cubic "%s" "%s" -srcnodata "-340282346638529993179660072199368212480.000" -dstnodata 0' % (Region.albers, self.scale, self.scale, elextents['xmin'], elextents['ymin'], elextents['xmax'], elextents['ymax'], elvrt, elfile)
        ...
        lcvrt = os.path.join(self.mapsdir, 'landcover.vrt')
        lcfile = os.path.join(self.mapsdir, 'landcover.tif')
        ...
        if Region.gdalwarp_broken_for_landcover:
            ...
            tifds = gdal.Open(lcvrt, GA_ReadOnly)
            ...
            # needs SRS
            warpcmd = 'gdalwarp -q -multi -tr %d %d -te %d %d %d %d -r near "%s" "%s"' % (self.scale, self.scale, lcextents['xmin'], lcextents['ymin'], lcextents['xmax'], lcextents['ymax'], lcvrt, lcfile)
