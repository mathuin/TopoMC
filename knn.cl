...
__kernel void knn(__global uint *indices, __global float *distances, __global uint *tree, __global float2 *coords, const uint lentree, const uint ink, const int xfirst, const uint xlen, const int xstep, const int yfirst, const uint ylen, const int ystep, const uint xmajor) {
  ...
  // xmajor -- INPUT: uint (1 if x major ordering, 0 if y major ordering)
  ...
  uint majorid, minorid;
  uint majorlen, minorlen;
  uint majorsize, minorsize;

  int xval, yval;

  // if xmajor then major = x
  if (xmajor == 1) {
    majorid = xid;
    majorlen = xlen;
    majorsize = xsize;
  ...
  for (uint majoridx = majorid; majoridx < majorlen; majoridx += majorsize) {
    for (uint minoridx = minorid; minoridx < minorlen; minoridx += minorsize) {
      if (xmajor == 1) {
	xval = xfirst + (majoridx * xstep);
	yval = yfirst + (minoridx * ystep);
      } else {
	xval = xfirst + (minoridx * xstep);
	yval = yfirst + (majoridx * ystep);
      }
      uint idx = majoridx * minorlen + minoridx;
#ifdef cl_intel_printf
      if (debugValues2 && idx == debugIdx or idx == debugIdx+1) {
	printf("idx=%d, majoridx=%d, minorlen=%d, minoridx=%d, xval=%d, yval=%d\n", idx, majoridx, minorlen, minoridx, xval, yval);
      }
#endif
	...
	if (debugOutput && idx == debugIdx) {
	  printf(" Neighbor %d for %d (%f, %f):\n", i, idx, queryPoint.x, queryPoint.y);
	}
#endif
#ifdef cl_intel_printf
	if (debugOutput && idx == debugIdx) {
...
__kernel void knnbase(__global uint *indices, __global float *distances, __global uint *tree, __global float2 *coords, const uint lentree, __global float2 *base, const uint lenbase, const uint ink) {
  // indices -- OUTPUT: uint array of shape (k*nelems)
  //            containing indices of the k nearest neighbors 
  //            for each element
  // distances -- OUTPUT: uint array of shape (k*nelems)
  ...
  for (uint idx = gid; idx < lenbase; idx += gsize) {
    ...
    // output results
    // neighbors in idx, idx+lenbase, idx+2*lenbase, ...
    for (uint i = 1; i <= countHeap; i++) {
      uint offset = (i-1)*lenbase;
#ifdef cl_intel_printf
      if (debugOutput && idx == debugIdx) {
	printf(" Neighbor %d for %d (%f, %f):\n", i, idx, queryPoint.x, queryPoint.y);
      }
