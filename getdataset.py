...
def checkInventory(args, EPSG):
    ...
    # for now, disable ND3 (the filenames don't have NED_ sigh)
    # desiredProductIDs = ['L01','NED','ND3']
    desiredProductIDs = ['L01','NED']
    productIDs = []
...
def downloadFile(downloadURL, datasetDir):
    ...
    try:
        ...
        downloadFile = open(os.path.join(datasetDir,filename), 'wb')
        while True:
            data = obj.read(8192)
            if data == "":
                break
...
def main(argv):
    ...
    for downloadURL in downloadURLs:
        downloadFile(downloadURL, datasetDir)
